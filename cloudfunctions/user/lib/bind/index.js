...
const main = async (event) => {
    ...
    try {
        ...
        const inviteResult = await transaction.collection('invites')
            ...
            .get();
        if (!inviteResult.data.length) {
            throw new error_1.AppError(error_1.ErrorCode.INVALID_PARAMS, '无效的邀请码');
        }
        // 检查是否已绑定
        const bindingResult = await transaction.collection('bindings')
            .where({
            userId: OPENID,
            status: 'active'
        })
            .get();
        if (bindingResult.data.length) {
            throw new error_1.AppError(error_1.ErrorCode.INVALID_PARAMS, '已绑定专家');
        }
        // 创建绑定关系

...
// 移除未导出的类型导入，因为已在本文件中定义了 SpecialistReport 接口
...
interface SpecialistWeeklyResult {
  success: boolean;
  report?: SpecialistReport;
  error?: string;
}

export const main = async (event: { specialistId: string }): Promise<SpecialistWeeklyResult> => {
  ...
  try {
    ...
    const bindings = (await db.collection('subscriptions')
      ...
      .get()) as IQueryResult<Binding>;

    const userIds = ((bindings?.data || []).map(binding => binding.userId)) || [];
    ...
    const measurements = (await db.collection('measurements')
      ...
      .get()) as IQueryResult<Measurement>;
    ...
    // 如果没有用户，返回空报告
    if (!userIds.length) {
      const emptyReport: SpecialistReport = {
        specialistId,
    ...
    measurements.data.forEach(measurement => {
      if (!userLatestData.has(measurement.userId)) {
        ...
        if (measurement.metrics.weightChange && measurement.metrics.weightChange > 2) {
          ...
          (riskWarning.abnormalIncrease.users as Array<{
            userId: string;
            name: string;
            weightChange: number;
          }>).push({
            userId: measurement.userId,
            name: measurement.userName,
